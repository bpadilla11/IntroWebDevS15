...
func index(res http.ResponseWriter, req *http.Request) {
	...
	if req.Method == "POST" {
		...
		if err != nil {
			...
			http.Redirect(res, req, `/?id=`+id, http.StatusSeeOther) // added id in parameter
			return
		}
...
func logout(res http.ResponseWriter, req *http.Request) {
	...
	http.Redirect(res, req, `/?id=`+cookie.Value, http.StatusSeeOther) // added id in the parameter
}
...
func login(res http.ResponseWriter, req *http.Request) {
	...
	if req.Method == "POST" && req.FormValue("password") == "secret" {
		...
		m.ID = id // added m.ID to id

		cookie, err := currentVisitor(m, req)
		if err != nil {
			log.Errorf(ctx, "ERROR login currentVisitor: %s", err)
			http.Redirect(res, req, `/?id=`+cookie.Value, http.StatusSeeOther) // added cookie.Value
			return
		}
		http.SetCookie(res, cookie)
